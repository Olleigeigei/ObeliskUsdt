...
- Node.js 18+
- 数据库：MySQL 或兼容方言；数据访问可传 `sequelize`（内置实现）或自实现 `ObeliskPersistence`（如 Prisma）
- Redis
- TronGrid / TronScan API Key

## 3. 安装

npm 包发布产物为 **JavaScript**，不包含 `.d.ts`；TypeScript 宿主需自行补充类型声明或按需封装。
...
使用 Prisma 接入数据层时，可参考仓库内 `examples/backend/prisma.schema.example.prisma` 与 `examples/backend/prisma-persistence.example.ts`，在宿主项目中生成 Client 后将 `createPrismaObeliskPersistence(prisma)` 作为 `initObeliskUSDT` 的 `persistence` 传入。
...
```ts
import { initObeliskUSDT, runObeliskUSDTMigrations } from '@obeliskstudio/obelisk-usdt';
...
// 迁移也可传 `query`（与 Sequelize.query 兼容的通用执行函数），与 `sequelize` 二选一。
...
- `runObeliskUSDTMigrations` 会自动读取包内 `database/migrations/*.sql`
- 自动创建迁移记录表 `obl_usdt_schema_migrations`
- 已执行迁移自动跳过
- 迁移不会在 `initObeliskUSDT()` 中自动执行，必须由宿主显式调用
- 初始化时 `sequelize` 与 `persistence` 二选一：`persistence` 为自实现数据访问层（表结构仍以包内 `obl_*` 为准）
...
- TronGrid 与 Tronscan 分别使用各自 API Key，并启用短熔断降频，避免第三方接口抖动拖垮主流程。

## 9.1 性能优化说明

- 钱包扫描启用固定并发池，提升多钱包扫描吞吐能力。
- 扫描配置读取采用短缓存，减少高频配置读取开销。
- 扫描间隔动态调整：检测到交易时自动提速，空闲时自动降频。
- 订单确认回调采用异步队列执行，支持重试与死信，避免慢回调阻塞扫描主流程。
- 通过 `GET /admin/payment/stats` 可查看扫描器健康指标（熔断状态、回调队列长度、死信数量）。
